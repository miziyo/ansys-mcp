"""G4 official Prime/MAPDL/DPF steady-thermal capability assessment."""
...
from ansys_research_runner.domain.capabilities import (
    ...
    SteadySolverGateCapabilityReport,
)
...
def collect_steady_solver_capabilities(
    *,
    probe_timeout_seconds: float = 180,
) -> SteadySolverGateCapabilityReport:
    """Run both required cases through the production Prime/MAPDL/DPF path."""
    ...
    if launch.status is CapabilityStatus.AVAILABLE:
        try:
            cases = run_actual_steady_gate_cases(probe_timeout_seconds=probe_timeout_seconds)
    ...
    if failure is not None:
        reasons.append(f"Actual steady solve failed: {failure}")
    if not worker_qualified:
        reasons.append(f"Required actual steady cases passed: {len(actual_cases)}/2.")
    launch_with_cases = launch.model_copy(
        update={"evidence": {**launch.evidence, "actual_cases": cases}}
    )
    return SteadySolverGateCapabilityReport(
...
def persist_steady_solver_capabilities(report: SteadySolverGateCapabilityReport) -> None:
    """Persist machine-local steady-solver evidence under the ignored runtime root."""

    paths = RunnerPaths.from_environment()
    paths.ensure_runtime()
    atomic_write_json(
        paths.runtime / "steady_solver_capability_report.json",
        report.model_dump(mode="json"),
    )
    atomic_write_json(
        paths.blockers / "G4.json",
        {
            "gate": "G4",
...
def load_steady_solver_capabilities(path: Path | None = None) -> SteadySolverGateCapabilityReport:
    """Load the last persisted G4 capability report."""

    target = path or RunnerPaths.from_environment().runtime / "steady_solver_capability_report.json"
    return SteadySolverGateCapabilityReport.model_validate_json(target.read_text(encoding="utf-8"))
